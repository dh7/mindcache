import { SystemTag } from 'mindcache';
...
export function TagFilter({
  ...
  const systemTags: SystemTag[] = ['SystemPrompt', 'LLMRead', 'LLMWrite', 'ApplyTemplate'];
      ...
      <div className="flex flex-wrap gap-2 items-center">
        {systemTags.map(tag => (
          <button
            ...
            className={`px-2 py-0.5 text-xs rounded transition border font-mono ${selectedSystemTags.includes(tag)
        ...
        <button
          ...
          className={`px-2 py-0.5 text-xs rounded transition border font-mono ${showUntagged
        ...
        {availableTags.map(tag => (
          <button
            ...
            className={`px-2 py-0.5 text-xs rounded transition border font-mono ${selectedTags.includes(tag)
              ? 'bg-cyan-900 bg-opacity-50 text-cyan-300 border-cyan-600'
              : 'bg-zinc-800 text-zinc-300 border-zinc-800 hover:bg-zinc-700'
            }`}
          >
